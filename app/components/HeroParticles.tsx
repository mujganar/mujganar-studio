...
// Base palette — will be tinted dynamically by shader
const PALETTE = [
  [0.184, 0.306, 0.157], // #4a7c3f dim green
  [0.478, 0.714, 0.282], // #7ab648 bright green
  [0.184, 0.384, 0.357], // #4a9b8e teal
  [0.141, 0.235, 0.118], // #243c1f very dim
  [0.220, 0.460, 0.420], // brighter teal-green
]
...
const DAMP       = 0.965
const DRIFT      = 0.00055
const SIDE_DRIFT = 0.00028  // side-bias force (spring will equilibrate to ~0.8 unit cluster)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      for (let i = 0; i < N; i++) {
        ...
        const z = (Math.random() - 0.5) * 0.6   // z spread for parallax depth
      ...
      const material = new THREE.ShaderMaterial({
        uniforms: {
          uTime:       { value: 0 },
          uSide:       { value: 0.5 },  // 0 = left/green, 1 = right/teal
        ...
        vertexShader: /* glsl */`
          ...
          void main() {
            // Side-reactive palette tint
            vec3 greenTint = vec3(0.478, 0.714, 0.282);  // #7ab648
      ...
      const onMouseMove = (e: MouseEvent) => {
        ...
        mouse.wx    = worldPt.x
        mouse.wy    = worldPt.y
        sideTarget  = e.clientX > rect.width / 2 ? 1 : 0
      }
      ...
      const animate = () => {
        ...
        // Smooth lerp toward target side (0.04 ≈ 25-frame transition)
        side += (sideTarget - side) * 0.04
        material.uniforms.uTime.value = t
        material.uniforms.uSide.value = side
        material.uniforms.uMouseWorld.value.set(mouse.wx, mouse.wy)

        // -1 = bias left, +1 = bias right
        const sideBias = (side - 0.5) * 2
        ...
        for (let i = 0; i < N; i++) {
          ...
          // Z-based speed factor for parallax (near particles drift faster)
          const zFactor = 0.65 + (pz + 0.3) * (0.35 / 0.3)

          // Organic Brownian drift
          velocities[i * 2]     += Math.sin(t * 0.38 + ph)       * DRIFT * zFactor
          velocities[i * 2 + 1] += Math.cos(t * 0.27 + ph * 1.3) * DRIFT * zFactor

          // Side bias — creates cluster drift toward active half
          velocities[i * 2] += sideBias * SIDE_DRIFT * zFactor
